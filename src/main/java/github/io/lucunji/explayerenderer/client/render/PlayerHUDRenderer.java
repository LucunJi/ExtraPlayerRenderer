...
public class PlayerHUDRenderer implements IRenderer {
    ...
    private static final List<DataBackupEntry<LivingEntity, ?>> LIVINGENTITY_BACKUP_ENTRIES = ImmutableList.of(
            ...
            new DataBackupEntry<LivingEntity, Float>(e -> e.pitch, (e, pitch) -> e.pitch = pitch),
    ...
    public void doRender(float partialTicks, MatrixStack matrixStack) {
        ...
        DataBackup<LivingEntity> backup = new DataBackup<>(targetEntity, LIVINGENTITY_BACKUP_ENTRIES);
        backup.save();
...
//                Configs.LIGHT_DEGREE.getDoubleValue(),
    ...
    private double getPoseOffsetY(LivingEntity targetEntity, float partialTicks, IConfigOptionListEntry poseOffsetMethod) {
        if (poseOffsetMethod == PoseOffsetMethod.AUTO) {
            final float defaultPlayerEyeHeight = 1.62f; /** see {@link PlayerEntity#getActiveEyeHeight} **/
            final float defaultPlayerSwimmingBBHeight = 0.6f; /** see {@link PlayerEntity#POSE_DIMENSIONS} **/
    ...
    private void transformEntity(LivingEntity targetEntity, float partialTicks, boolean forceStanding) {
        ...
        if (!targetEntity.isSwimming() && !targetEntity.isFallFlying() && !isCrawling(targetEntity)) {
        ...
        targetEntity.prevPitch = targetEntity.pitch = (float) (MathHelper.clamp(
                        MathHelper.lerp(partialTicks, targetEntity.prevPitch, targetEntity.pitch),
    ...
    private void performRendering(Entity targetEntity, double posX, double posY, double size, boolean mirror,
                                  double poseOffsetY, IConfigOptionListEntry poseOffsetMethod,
//                                  double lightDegree,
                                  float partialTicks) {
    ...
    private static boolean isCrawling(LivingEntity entity) {
        return entity.isInSwimmingPose() && !entity.isTouchingWater();
    }
